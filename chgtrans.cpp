...
for (int i = 0; i < good0.size(); i ++)
{
	...
	if (i != 0 && i % (nx0*ny0)  == 0) 
	{
		if (i == nx0*ny0)
		{
		esp0[counter0] = (sum0 - good0[i - 1])/(nx0*ny0);
		counter0 ++;
		sum0 = 0;
		}
		else
		{
		esp0[counter0] = sum0/(nx0*ny0);
                counter0 ++;
                sum0 = 0;
		}
	}
} 
cout << "The vacuum energy is " << max(esp0, nz0) << " eV." << endl;
...
for (int i = 0; i < good1.size(); i ++)
{
        ...
        if (i != 0 && i % (nx1*ny1)  == 0)
        {
                if (i == nx1*ny1)
                {
                esp1[counter1] = (sum1 - good1[i - 1])/(nx1*ny1);
                counter1 ++;
                sum1 = 0;
                }
                else
                {
                esp1[counter1] = sum1/(nx1*ny1);
                counter1 ++;
                sum1 = 0;
                }
        }
...
cout << "The vacuum energy is " << max(esp1, nz1) << " eV." << endl;
...
for (int i = 0; i < good2.size(); i ++)
{
        ...
        if (i != 0 && i % (nx2*ny2)  == 0)
        {
                if (i == nx2*ny2)
                {
                esp2[counter2] = (sum2 - good2[i - 1])/(nx2*ny2);
                counter2 ++;
                sum2 = 0;
                }
                else
                {
                esp2[counter2] = sum2/(nx2*ny2);
                counter2 ++;
                sum2 = 0;
                }
        }
